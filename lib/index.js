...
var retire_check = function retire_check(plugin_config) {
  ...
  return vile.spawn(RETIRE_BIN, opts).then(read_report);
};
...
var full_dep_path_parent = function full_dep_path_parent(parent) {
  var sub_parent = _.get(parent, "parent");
  var name = _.get(parent, "component");
  var version = _.get(parent, "version");
  if (sub_parent) {
    return full_dep_path_parent(sub_parent) + (" > " + name + "@" + version);
  } else {
    return name + "@" + version;
  }
};

var dependency_path = function dependency_path(result) {
  var parent = _.get(result, "parent");
  if (!parent) return;
  return full_dep_path_parent(parent);
};

var vile_issue = function vile_issue(vulnerability, result, file) {
  var severity = _.get(vulnerability, "severity");
  var desc = description(vulnerability);
  var dep_path = dependency_path(result);
...
var into_issues = function into_issues(reports) {
  ...
  var map_by_name = _.reduce(results, function (map, result) {
    ...
    var level = _.get(result, "level", 1);
  ...
  _.each(map_by_name, function (versions, name) {
    _.each(versions, function (levels, version) {
      // if file type then no deepest level here- just assume 1
      var deepest_level = _.reduce(levels, function (deepest, result, level) {
        if (level > deepest) deepest = level;
        return deepest;
      }, 1);

      var results = levels[deepest_level];
